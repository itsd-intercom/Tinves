                        ...
                        <tbody>
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>{{ $item->status }}</td>
                                    <td>
                                        {{-- <button class="btn btn-primary btn-edit" data-id="1" data-toggle="modal" data-target="#modalEdit">Edit</button> --}}
                                        <a class="btn btn-info tbn-sm btn-edit" data-id="{{ $item->id }} title="Edit Data" > <i class="fas fa-edit"></i></a>
                                        {{-- <a href="{{ route('fc.del  ete', $item->id) }}" class="btn btn-danger sm" title="Delete" id="delete"> <i class="fas fa-trash-alt"></i></a> --}}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                                            ...
                                            <div class="row mb-2">
                                                ...
                                                <div class="col-5 mb-2">
                                                    <label for="date" class="col-sm-4 col-form-label">Jenis</label>
                                                        <select name="jenis" id="jenis" class="form-select" aria-label="Default select example" onchange="showFields(this)">
                                                        <option selected="">Open this select menu</option>
        ...
        $('.btn-edit').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Panggil event change pada jenis.edit
                    // $('#formEdit').find('#jenis').trigger('change');
                    ...
                    $('#formEdit').find('#akta_pendirian').val(response.data.akta_pendirian);
                    $('#formEdit').find('#sk_kemenkumham').val(response.data.sk_kemenkumham);
                    $('#formEdit').find('#akta_perubahan').val(response.data.akta_perubahan);
                    $('#formEdit').find('#nib').val(response.data.nib);
                    $('#formEdit').find('#rek_koran_perusahaan').val(response.data.rek_koran_perusahaan);
                    $('#formEdit').find('#ktp_direksi').val(response.data.ktp_direksi);
                    $('#formEdit').find('#ktp_komisaris').val(response.data.ktp_komisaris);
                    $('#formEdit').find('#ktp_p_saham').val(response.data.ktp_p_saham);
            ...
            // Dapatkan elemen select jenis.edit
            const jenis = document.getElementById('jenis');
                    // Dapatkan elemen form pribadi
                    const pribadiFields = document.getElementById('pribadiFields');
                    const perusahaanFields = document.getElementById('perusahaanFields');

                    // Tambahkan event listener ke elemen jenis.edit
                    jenis.addEventListener('change', function () {
                        if (jenis.value === 'Pribadi') {
                            pribadiFields.style.display = 'block'; // Tampilkan form pribadi
                            perusahaanFields.style.display = 'none'; // Sembunyikan form perusahaan
                        } else if (jenis.value === 'Perusahaan') {
                            pribadiFields.style.display = 'none'; // Sembunyikan form pribadi jika bukan "Pribadi"
                            perusahaanFields.style.display = 'block'; // Tampilkan form perusahaan jika "Perusahaan"
                        } else {
                            pribadiFields.style.display = 'none'; // Sembunyikan form pribadi jika bukan "Pribadi" atau "Perusahaan"
                            perusahaanFields.style.display = 'none'; // Sembunyikan form perusahaan jika bukan "Pribadi" atau "Perusahaan"
                        }
                    });
                });
</script>
